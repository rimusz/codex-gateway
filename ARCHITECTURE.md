...
| Anthropic Claude Code login | `ClaudeCodeSession` (env `CLAUDE_CODE_OAUTH_TOKEN`, `~/.claude/.credentials.json`, `~/.claude.json`, macOS Keychain `Claude Code-credentials`). Loads every source, then uses the first non-expired `sk-ant-oat…` token — a stale credentials file must not hide a refreshed Keychain login (Claude CLI often updates Keychain only). `ProviderAuth` applies Bearer + `anthropic-version` + `anthropic-beta: oauth-2025-04-20` (no `x-api-key`) to the same `https://api.anthropic.com/v1` OpenAI-compat routes. Parallel to **Anthropic (Claude)** Console-key preset. Settings / Setup / Doctor run `ClaudeCodeSession.status()` off the UI thread (`Task.detached`) and publish back on `MainActor`; `installPreset(.claudeCode)` does one sync `status()` for the install toast. Keychain probe is `#if os(macOS)` — Linux/CI must use fixtures, not a faked local login. |
