...
│   │   ├── ProviderPresets.swift # Built-in presets (incl. Anthropic Console + Claude Code, Cursor bridge, xAI API + Grok OAuth)
...
│   │   ├── ClaudeCodeSession.swift # Local Claude Code / CLI login (env, ~/.claude, macOS Keychain)
...
│   │   ├── DoctorReport.swift    # Pure Doctor check mapping (gateway, Codex, Node, Cursor, Grok, Claude Code)
│   │   ├── DoctorCollector.swift # Live probes for DoctorInputs
│   │   ├── ProviderModelFetcher.swift # OpenAI /models + Anthropic GET {base}/models (Console + Claude Code) + Cline Pass recommended-models
...
| Path | Purpose |
|------|---------|
| `~/.codexgateway/custom_model_catalog.json` | CodexGateway internal model catalog (routing metadata). Raw model ids get friendly display names via `ModelCatalog.prettyDisplayName` / `normalizeDisplayNames` (run on Settings reload + gateway startup; drops `vendor/model` prefixes, collapses doubled vendors, title-cases, prefixes the provider brand Cline-style via `providerBrand`, and appends `(API)` / `(OAuth)` for xAI vs Grok OAuth and Anthropic Console vs Claude Code, e.g. "xAI Grok 4.5 (API)", "Anthropic Claude Sonnet 5 (OAuth)"); user-edited names are preserved. `ProviderConfig.displayLabel` prefers a stored `display_name` over the built-in preset title. Providers and models are listed A–Z by display name in Settings; **Add Provider** (presets + custom add) starts collapsed; the **Providers** and **Models** sections are collapsible (expanded by default). Custom Codex picker entries are exported in that order. |
...
| `~/.codexgateway/providers.json` | Provider endpoints + credentials. Optional `auth_kind`: omitted/`api_key` → Bearer key to `{base_url}/chat/completions`; `anthropic` → Bearer plus `x-api-key` and `anthropic-version: 2023-06-01` to Anthropic's OpenAI-compat layer (`https://api.anthropic.com/v1` `/chat/completions` and `/models`); `claude_code` → same OpenAI-compat base, but Bearer + `anthropic-beta: oauth-2025-04-20` from the live Claude Code session (no token stored); `grok_oauth` → `GrokOAuthClient` (no key stored). Read **live** by the gateway per request (`ModelCatalog.resolveUpstream`), so provider/preset changes take effect immediately — **no Codex restart** (only model changes require one; see `SettingsStore.requiresCodexRestart`). |
| `~/.grok/auth.json` | Official Grok CLI OAuth session (not owned by CodexGateway). Used when a provider has `auth_kind = grok_oauth`. |
| `~/.claude/.credentials.json` / `~/.claude.json` | Claude Code / Claude CLI login (not owned by CodexGateway). Used when a provider has `auth_kind = claude_code`. macOS also probes Keychain service `Claude Code-credentials`. Tokens are never written to git or `providers.json`. |
| `~/.codexgateway/fetched_models.json` | Cached model lists per provider (OpenAI `/models`, Anthropic `GET {base_url}/models` → `https://api.anthropic.com/v1/models` for both Console and Claude Code, or Cline Pass recommended-models feed); replaced on each fetch |
...
| Open Doctor | `DoctorWindowController` + `DoctorView`; menu **Doctor…** (⌘D); Settings toolbar **Doctor**. Fixed-size window (close only). `DoctorReport` maps `DoctorInputs` (pure); `DoctorCollector` probes `/health`, Codex config/sign-in, Node ≥ 22.13, Cursor key/sidecar, Grok OAuth, Claude Code login. Grok/Claude Code session probes run off the main actor (`Task.detached`). Once managed markers show that Gateway was applied, it detects conflicting keys/tables outside those markers; native configs without managed markers are never flagged. **Repair and Restart Codex** performs catalog/config writes and the synchronous restart off the main thread, restarting only after successful writes. |
...
| Anthropic Claude Code login | `ClaudeCodeSession` (env `CLAUDE_CODE_OAUTH_TOKEN`, `~/.claude/.credentials.json`, `~/.claude.json`, macOS Keychain `Claude Code-credentials`). Walks sources in order and stops at the first usable `sk-ant-oat…` token — a stale credentials file must not hide a refreshed Keychain login, and a usable env/file token must not spawn `security`. Status hints come from the highest-priority unusable source. `ProviderAuth` applies Bearer + `anthropic-version` + `anthropic-beta: oauth-2025-04-20` (no `x-api-key`) to the same `https://api.anthropic.com/v1` OpenAI-compat routes. Parallel to **Anthropic (Claude)** Console-key preset. Settings / Setup / Doctor run `ClaudeCodeSession.status()` off the UI thread (`Task.detached`) and publish back on `MainActor`; `installPreset(.claudeCode)` does one sync `status()` for the install toast. Keychain probe is `#if os(macOS)` — Linux/CI must use fixtures, not a faked local login. |
...
| Reset/Update gateway config | `SettingsView` (label toggles on `SettingsStore.gatewayConfigInSync`), `SettingsStore.resetGatewayConfig` / `applyGatewayConfig` / `updateGatewayConfig` (apply + restart), `CodexConfig.resetToNative` (Codex-side only; keeps `~/.codexgateway` data). `CodexConfig.ensureConfigFile()` creates a missing `config.toml` on explicit apply. Apply/reset also remove stale unmarked gateway root keys and `codexgateway` / legacy `codexbar` provider tables before writing, preventing invalid duplicate TOML after another config writer re-serializes managed values. |
