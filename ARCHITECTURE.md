...
| Anthropic Claude Code login | `ClaudeCodeSession` (env `CLAUDE_CODE_OAUTH_TOKEN`, `~/.claude/.credentials.json`, `~/.claude.json`, macOS Keychain `Claude Code-credentials`). `ProviderAuth` applies Bearer + `anthropic-version` + `anthropic-beta: oauth-2025-04-20` (no `x-api-key`) to the same `https://api.anthropic.com/v1` OpenAI-compat routes. Parallel to **Anthropic (Claude)** Console-key preset. Keychain probe is `#if os(macOS)` — Linux/CI must use fixtures, not a faked local login. |
