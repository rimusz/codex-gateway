...
│   │   ├── ProviderModelFetcher.swift # OpenAI /models + Anthropic GET {base}/models + Cline Pass recommended-models
...
| `~/.codexgateway/fetched_models.json` | Cached model lists per provider (OpenAI `/models`, Anthropic `GET {base_url}/models` → `https://api.anthropic.com/v1/models`, or Cline Pass recommended-models feed); replaced on each fetch |
