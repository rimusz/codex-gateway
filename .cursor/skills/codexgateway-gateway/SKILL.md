...
| `~/.codexgateway/providers.json` | `ModelCatalog` (`auth_kind` optional: `api_key` / `anthropic` / `claude_code` / `grok_oauth` / `cursor_bridge`) |
| `~/.claude/.credentials.json` | Claude Code login (read by `ClaudeCodeSession` when `auth_kind` is `claude_code`; never stored) |
