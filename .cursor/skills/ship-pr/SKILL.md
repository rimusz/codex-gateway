...
2. Run the project’s required verification before the first push when the repo defines it (this repo: `make test`; Swift/app code changes also need `make run` + Computer Use per `.cursor/rules/docs-and-tests.mdc`).
...
1. Stage relevant files only (no secrets: `.env`, credentials, private keys, Cursor API keys).
...
## This repo (CodexGateway)

- Branch from `main`; PR against `main`.
- Before first push of app changes: `make test` (+ `make run` and Computer Use when Swift/app binary changed).
- Prefer existing `.cursor/skills/codexgateway-*` for build/release/gateway details.
