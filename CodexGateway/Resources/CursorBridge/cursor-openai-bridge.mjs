#!/usr/bin/env node
/**
 * Minimal OpenAI-compatible /v1 bridge for CodexGateway.
 *
 * Uses @cursor/sdk with CURSOR_API_KEY. Speaks GET /v1/models and
 * POST /v1/chat/completions so Codex config base_url can point at localhost.
...
const port = parseInt(process.env.CURSOR_BRIDGE_PORT || "18788", 10);
...
const workspaceCwd = path.resolve(
  process.env.CURSOR_BRIDGE_CWD ||
    path.join(os.homedir(), "Library", "Application Support", "CodexGateway", "cursor-bridge-workspace")
);
...
server.listen(port, host, () => {
  console.log(`CodexGateway Cursor bridge listening on http://${host}:${port}/v1`);
...
async function handleRequest(request, response) {
  ...
  if (request.method === "GET" && (apiPath === "/health" || apiPath === "/")) {
    writeJson(response, { ok: true, service: "codexgateway-cursor-bridge", cwd: workspaceCwd });
    return;
  }
...
async function runAgent({ apiKey, model, prompt, onDelta }) {
  const agent = await Agent.create({
    apiKey,
    model: { id: model },
    name: "CodexGateway Cursor bridge",
...
function buildChatPrompt(messages) {
  const lines = [
    "You are answering through a local OpenAI-compatible bridge used by CodexGateway.",
...
/** Routing aliases — not real models users should add in CodexGateway. */
