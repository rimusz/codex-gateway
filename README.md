...
- **Anthropic (Claude Code)** uses **Use Claude Code login**: CodexGateway reads the local Claude Code / CLI session at request time (`CLAUDE_CODE_OAUTH_TOKEN`, `~/.claude/.credentials.json`, `~/.claude.json`, or the macOS Keychain item `Claude Code-credentials`) and uses the first non-expired OAuth token, so a stale credentials file does not hide a refreshed Keychain login. No OAuth token is stored in `providers.json` or typed into Settings. Requests send Bearer plus `anthropic-version` and `anthropic-beta: oauth-2025-04-20` (no `x-api-key`). On the Mac, run `claude auth login` or Claude Code `/login` first. Keychain lookup is macOS-only.
